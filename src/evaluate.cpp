...
namespace {
  ...
  constexpr Value LazyThreshold  = Value(1220);
...
#undef S
  ...
  Value Evaluation<T>::value() {
    ...
    if (abs(v) > LazyThreshold + pos.non_pawn_material() / 64 + (pos.count<QUEEN>() > 0) * 200)
    {
        bool dangerousPPawn = bool(pe->passed_pawns(WHITE) & (Rank7BB | Rank6BB))
