...
#undef S
  ...
  Score Evaluation<T>::king() const {
    ...
    if (rookChecks)
        kingDanger += SafeCheck[ROOK][more_than_one(rookChecks)] / (1 + StM);
    else
    ...
    if (queenChecks)
        kingDanger += SafeCheck[QUEEN][more_than_one(queenChecks)] / (1 + StM);
    ...
    bishopChecks =  b2 & attackedBy[Them][BISHOP] & safe
                  & ~queenChecks;
    if (bishopChecks)
        kingDanger += SafeCheck[BISHOP][more_than_one(bishopChecks)] / (1 + StM);
    ...
    if (knightChecks & safe)
        kingDanger += SafeCheck[KNIGHT][more_than_one(knightChecks & safe)] / (1 + StM);
    else
