  ...
  Score Evaluation<T>::passed() const {
    ...
    while (b)
    {
        ...
        if (w)
        {
            ...
            bonus += make_score((  king_proximity(Them, blockSq) * 5 - king_proximity(Us,   blockSq) * 2) * w / 4
									, (  king_proximity(Them, blockSq) * 5 - king_proximity(Us,   blockSq) * 2) * w);

            // If blockSq is not the queening square then consider also a second push
            if (r != RANK_7)
                bonus -= make_score(king_proximity(Us, blockSq + Up) * w / 4, king_proximity(Us, blockSq + Up) * w);
