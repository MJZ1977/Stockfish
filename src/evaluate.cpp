...
namespace {
  ...
  constexpr Value NNUEThreshold1 =   Value(350);
  constexpr Value NNUEThreshold2 =   Value(50);
...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      bool  largePsq = psq * 16 > (NNUEThreshold1 + pos.non_pawn_material() / 64 + pos.this_thread()->nnue_gap / 2) * r50;
      ...
      if (   largePsq
          && (abs(v) * 16 < NNUEThreshold2 * r50 + pos.this_thread()->nnue_gap / 4
          || (   pos.opposite_bishops()
              && abs(v) * 16 < (NNUEThreshold1 + pos.non_pawn_material() / 64 + pos.this_thread()->nnue_gap / 2) * r50
              && !(pos.this_thread()->nodes & 0xB))))
      {
          Value v2 = adjusted_NNUE();
          pos.this_thread()->nnue_gap = (pos.this_thread()->nnue_gap * 15 + abs(v-v2)) / 16;
          v = v2;
      }
  }
