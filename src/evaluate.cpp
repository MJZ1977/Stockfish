...
#undef S
  ...
  Score Evaluation<T>::king() const {
    ...
    // Easy escape squares
    b1 = attackedBy[Us][KING] & ~attackedBy[Them][ALL_PIECES] & ~(pos.pieces(Us,PAWN) & shift<Down>(pos.pieces()));
    no_escape = !bool(b1);
    ...
    if (kingDanger > 100)
        score -= make_score(kingDanger * kingDanger / 4096, kingDanger / 16 + 8 * no_escape);
