...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score QueenPinPiece      = S( 70, 16);
  constexpr Score QueenPinPawn       = S( 47, 14);
  constexpr Score QueenDiscovered    = S( 52, 15);
  ...
  Score Evaluation<T>::pieces() {
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == QUEEN)
        {
            ...
            while (Qblockers)
            {
                Piece blocker = pos.piece_on(pop_lsb(&Qblockers));
                if (color_of(blocker) == Them)
                  score -= QueenDiscovered;
                else if (type_of(blocker) != PAWN)
                  score -= QueenPinPiece;
                else
                  score -= QueenPinPawn;
             }
