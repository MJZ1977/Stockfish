...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   std::min(ss->ply,pos.rule50_count()) > 18 + 6 * (pos.non_pawn_material() < 6000)
        && ss->ksq == (ss-(18 + 6 * (pos.non_pawn_material() < 6000)))->ksq
        && beta < 0
        && (depth < 8 || std::min(ss->ply,pos.rule50_count()) > 36))
    {
