...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    posKey = (pos.key() ^ Key((std::min(pos.rule50_count(), ss->ply) > 29) << 17)) ^ Key(excludedMove << 16); // Isn't a very good hash
...
moves_loop: // When in check, search starts from here
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (   pos.is_draw(ss->ply, pos.count<ALL_PIECES>() < 8 ? 99 : 41)
        || ss->ply >= MAX_PLY)
    ...
    posKey = pos.key() ^ Key((std::min(pos.rule50_count(), ss->ply) > 29) << 17);
    tte = TT.probe(posKey, ttHit);
    ttValue = ttHit ? value_from_tt(tte->value(), ss->ply) : VALUE_NONE;
    ttMove = ttHit ? tte->move() : MOVE_NONE;
