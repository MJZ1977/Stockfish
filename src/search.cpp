...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (rootNode && moveCount > 1)
      {
          ...
          candidateMove = rm.nodesSearched > (10 + thisThread->rootMoves[0].nodesSearched / (2 + depth * 5 / ONE_PLY / 16));
          /*if (candidateMove)
      ...
      if (doFullDepthSearch && !candidateMove)
          value = -search<NonPV>(pos, ss+1, -(alpha+1), -alpha, newDepth, !cutNode);

      // For PV nodes only, do a full PV search on the first move or after a fail
      // high (in the latter case search only if value < beta), otherwise let the
      // parent node fail low with value <= alpha and try another move.
      if (PvNode && (moveCount == 1 || candidateMove || (value > alpha && (rootNode || value < beta))))
      {
          (ss+1)->pv = pv;
          (ss+1)->pv[0] = MOVE_NONE;
