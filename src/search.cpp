    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_quiet_stats(pos, ss, bestMove, quietsSearched, quietCount,
                               stat_bonus(depth + (bestValue > beta + PawnValueMg) + (bestValue >= beta && givesCheck)));

        update_capture_stats(pos, bestMove, capturesSearched, captureCount, stat_bonus(depth + 1 + (bestValue >= beta && givesCheck)));
