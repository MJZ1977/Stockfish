...
void Thread::search() {
  ...
  int maximal_depth = 6 + std::min(int(0.4*pow(Time.optimum(),0.25)*(10+(100-Gm_ph)/10)), MAX_PLY);
  int pvBonus = 0;
          ...
          && !Threads.stopOnPonderhit)
          {
              ...
              if (   rootMoves.size() == 1
                  ...
                  || (completedDepth >= (maximal_depth/2)
                    && rootMoves[0].score >= rootMoves[1].previousScore + stop_strat(200,600,maximal_depth-completedDepth)
