...
namespace {
  ...
  constexpr double MaxRatio   = 8.4;  // When in trouble, we can step over reserved time with this ratio
  constexpr double StealRatio = 0.4; // However we must not steal time from remaining moves over this ratio
  ...
  TimePoint remaining(TimePoint myTime, int movesToGo, int ply, TimePoint slowMover) {
    ...
    constexpr double TMaxRatio   = (T == OptimumTime ? 1.05 : MaxRatio);
    constexpr double TStealRatio = (T == OptimumTime ? 0.01 : StealRatio);
